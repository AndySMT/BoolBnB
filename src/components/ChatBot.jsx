...
import { baseUrl, chatAi } from '../globals/apiUrls';

function ChatBot({ propertyId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setMessages([
          ...newMessages,
          { role: 'ai', content: 'Sorry, I could not process your request.' }
        ]);
      }
    } catch (error) {
    ...
    <div className="md:block">
      ...
      {isOpen && (
        <div className="fixed bottom-[9rem] md:bottom-[calc(4rem+1.5rem)] right-0 mr-4 bg-white p-6 rounded-lg border border-[#e5e7eb] w-[350px] md:w-[430px] h-[500px] md:h-[600px] shadow-sm z-50">
          ...
          <form onSubmit={handleSubmit} className="flex items-center pt-0">
            <div className="flex items-center justify-center w-full space-x-2">
              ...
              <button
                type="submit"
                className="inline-flex items-center justify-center rounded-md text-sm font-medium text-[#f9fafb] disabled:pointer-events-none disabled:opacity-50 bg-black hover:bg-[#111827E6] h-10 px-4 py-2 cursor-pointer"
              >
                Invia
              </button>
            </div>
